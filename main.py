...
parser.add_argument('--c_p', type=str, default='configs')
parser.add_argument('--version', type=str, default='ce')
parser.add_argument('--exp_n', type=str, default="run1")
parser.add_argument('--load_model', action='store_true', default=False)
parser.add_argument('--data_parallel', action='store_true', default=False)
parser.add_argument('--asym', action='store_true', default=False)
parser.add_argument('--n_r', type=float, default=0.0)
args = parser.parse_args()


def main(version):
    # Set up
    if args.exp_n == '' or args.exp_n is None:
        args.exp_n = 'exp_' + datetime.datetime.now()
    exp_path = os.path.join('/output', args.exp_n, version)
    log_file_path = os.path.join(exp_path, version)
    checkpoint_path = os.path.join(exp_path, 'checkpoints')
    checkpoint_path_file = os.path.join(checkpoint_path, version)
    #writer_path=os.path.join(exp_path, 'writer')
    util.build_dirs(exp_path)
    util.build_dirs(checkpoint_path)
    #util.build_dirs(writer_path)
    #writer = SummaryWriter(writer_path)

    logger = util.setup_logger(name=version, log_file=log_file_path + ".log")
    ...
    config_file = os.path.join(args.c_p, version) + '.yaml'
    config = mlconfig.load(config_file)
    config.set_immutable()
    shutil.copyfile(config_file, os.path.join(exp_path, version+'.yaml'))
    for key in config:
        logger.info("%s: %s" % (key, config[key]))

    # data
    if config.dataset.name == 'DatasetGenerator':
        data_loader = config.dataset(seed=args.seed, noise_rate=args.n_r, asym=args.asym)
    else:
